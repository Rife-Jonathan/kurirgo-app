  ...
  return (
    <div className="w-full">
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-10 bg-white rounded-2xl border border-gray-100">No orders found.</div>
      ) : (
            ...
            <tbody>
              {orders.map(order => (
                <tr key={order.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  ...
                  <td className="p-4 text-sm text-gray-600 truncate max-w-[200px]">{order.destAddress}</td>
                  ...
                  <td className="p-4 text-sm">
                    <Link to={"/sender/order/" + (order.id) + ""} className="text-blue-600 font-medium hover:underline">View</Link>
                  </td>
                </tr>
              ))}
            </tbody>
